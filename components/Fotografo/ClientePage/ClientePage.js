...
import { View, Text, Image, TouchableOpacity, Modal, Linking, ScrollView, FlatList, Alert } from 'react-native';
...
const TelaFotografo = ({ navigation }) => {
  ...
  const fetchJobsCliente = async () => {
    try {
      const response = await axios.get(`${API_URL_MOBILE}/jobsCliente/${user.user.id}`);
      setJobData(response.data.jobs);
    } catch (error) {
      console.error("Erro ao coletar informações sobre os jobs", error);
      Alert.alert("Erro", "Erro ao coletar informações sobre os jobs");
    }
  };
